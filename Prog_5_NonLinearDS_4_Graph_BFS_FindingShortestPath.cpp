...
private:
    ...
    int *_pShortestDistance = nullptr;
    int *_pPreceedingNode = nullptr;
...
Graph::~Graph()
{
    if (_pShortestDistance)
        delete[] _pShortestDistance;

    if (_pPreceedingNode)
        delete[] _pPreceedingNode;
...
bool Graph::BFS(int iSource, int iDestination)
{
    ...
    while (!q.empty())
    {
        ...
        while (size)
        {
            ...
            {
                if (!_bIsVisited[iAdjNode])
                {
                    _bIsVisited[iAdjNode] = true;
                    q.push(iAdjNode);
